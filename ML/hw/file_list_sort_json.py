...
for i in range(len(file_tuple[2])):
	...
	if i == 0:
		file_list.append(files(FileName[i],int(FileName[i][0:3:1]+FileName[i][5:6:1]+FileName[i][8:9:1])))
	else:
		for j in range(len(file_list)):
			if file_list[j].date > int(FileName[i][0:3:1]+FileName[i][5:6:1]+FileName[i][8:9:1]):
				file_list.insert(j,files(FileName[i],int(FileName[i][0:3:1]+FileName[i][5:6:1]+FileName[i][8:9:1])))
				break
