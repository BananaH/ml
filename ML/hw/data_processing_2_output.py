...
for i in range(len(Stock_con)-1):
	for j in range(len(Stock_con[i])):
		if NameCmp(Stock_con[i][j].name,null_list) :
	...
	for j in range(len(Stock_con[i])):
		if (Stock_con[i][j].up==99)&(NameCmp(Stock_con[i][j].name,null_list)==1):
			null_list.append(Stock_con[i][j].name)

for i in range(len(Stock_con)-1):
	for j in range(len(Stock_con[i])):
		if NameCmp(Stock_con[i][j].name,null_list):
			fout.write(str(Stock_con[i][j].up)+"1:"+str(Stock_con[i][j].opening)+"2:"+str(Stock_con[i][j].closing)+"3:"+str(Stock_con[i][j].highest)+"4:"+str(Stock_con[i][j].lowest)+"5:"+str(Stock_con[i][j].volume)+"6"+str(Stock_con[i][j].adj_close)+"\n")
		#	print Stock_con[i][j].up,"1:",Stock_con[i][j].opening,"2:",Stock_con[i][j].closing,"3:",Stock_con[i][j].highest,"4:",Stock_con[i][j].lowest,"5:",Stock_con[i][j].volume,"6",Stock_con[i][j].adj_close
