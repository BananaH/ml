...
def find_file(list,target):
	for i in range(len(list)):
		if(list[len(list)-1-i].name==target):
			return int(len(list)-i-1)
		elif(list[len(list)-1-i].name[5:7]==target[5:7]) & (list[len(list)-1-i].name[8:10]<target[8:10]) | (list[len(list)-1-i].name[5:7]<target[5:7]):
...
#user input #
target = sys.argv[1]
predict =target[0:4:1]+'-'+target[5:7:1]+'-'+target[8:10:1]+'.json'
days = int(sys.argv[2])
predict_list = sys.argv[3]
...
print file_list[flag].name,file_list[flag_e].name
if flag<5:
	print "No more previous day."
else:
	flag_s=flag - days
while (flag_s!=(flag_e+1)) & (flag_s!=-1) & (flag_e!=-1):
...
for i in range(len(Stock_con)-days):
	for k in range(len(Stock_con[i+days])):
		if Stock_con[i+days][k].name != 0:
			temp = Stock_con[i+days][k].name
			for j in range(days):
				...
				if find_stock(Stock_con[i+j],temp)==-1:
					...
					if j==(days-9):
						...
						if nine_max<Stock_con[i+j][label[j]].highest:
							nine_max=Stock_con[i+j][label[j]].highest
				#	buff += str(Stock_con[i+j][label[j]].date)
				#	buff=str(Stock_con[i+j][label[j]].date)+" "+str(Stock_con[i+j][label[j]].name)+" "+str(count)+":"+str(Stock_con[i+j][label[j]].opening)+" "+str(count+1)+":"+str(Stock_con[i+j][label[j]].closing)+" "+str(count+2)+":"+str(Stock_con[i+j][label[j]].highest)+" "+str(count+3)+":"+str(Stock_con[i+j][label[j]].lowest)+" "+str(count+4)+":"+str(Stock_con[i+j][label[j]].volume)+" "+str(count+5)+":"+str(Stock_con[i+j][label[j]].adj_close)+" "+buff
					if j ==days-1:
						rsv = ((Stock_con[i+j][label[j]].closing - nine_min )/(nine_max-nine_min))*100
					#	print Stock_con[i+j][label[j]].date,Stock_con[i+j][label[j]].name,rsv,nine_min,nine_max
						TK=(2*YK/3)+(rsv/3)
						TD=(2*YD/3)+(TK/3)
						KD=TK-TD
						KD_diff=KD-KDY
					...
					#	fout.write(buff+"\n")
						...
						f_stock_list.write(str(Stock_con[i+j][label[j]].name)+" "+str(Stock_con[i+j][label[j]].opening)+" "+str(Stock_con[i+j][label[j]].closing)+" "+str(Stock_con[i+j][label[j]].highest)+" "+str(Stock_con[i+j][label[j]].lowest)+" "+str(Stock_con[i+j][label[j]].volume)+" "+str(Stock_con[i+j][label[j]].adj_close)+" "+"\n")
						buff = ""
						YD=TD
						TD=0
						YK=TK
